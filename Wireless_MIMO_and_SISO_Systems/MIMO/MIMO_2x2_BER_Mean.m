...
%Channel Selection 
%****************************************************************************
Channel_mode = 'Rayleigh' ; %Select: 'Rayleigh' or 'Rician'
%****************************************************************************
...
for m = 1:length(EbNoVec)
    ...
    while errorStats(2) <= maxBitErrors && errorStats(3) <= maxNumBits
        ...
        qamTx = Modulation(dataIn);                   % Apply QAM modulation
        txSig1 = ofdmMod(qamTx);                      % Apply OFDM modulation
        txSig2 = ofdmMod(qamTx);
        ...
        path1 = rxSig_eq1(:,2);                       %Path selection
        ...
        Mean = (qamRx1 + qamRx2)/2;                   %Average of OFDM outputs to improve BER
        
        dataOut = Demodulation(Mean);                 % Apply QAM demodulation             
